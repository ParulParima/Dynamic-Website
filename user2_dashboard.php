...
function k($row)
	{
		?>
	<h5><b><u>CUSTOMER's DETAILS</u></b></h5>
				<table>
				<tr>
			   ...
			   <br>
			   <input type="hidden" id="phpd" name="phpd" value="2">
			   </br>
...
<nav class="navbar navbar-light bg-dark justify-content-between">
   <a class="logo" >A.K.ROY & ASSOCIATES</a>
    ...
    <div class="col-sm-10">
	    <center><a><b>WELCOME TO YOUR DASHBOARD USER2</b></a></center>
		<div id="demo">
			<?php 
				if(isset($_POST['new_data']))
				{
				  ...
				  <td><input type="text" placeholder="ENTER PHONE NUMBER" name="cphn" required></td>
				   ...
				   <tr>
					...
					<br>
				 <input class="btn btn-outline-success btn-sm my-sm-0 bg-dark ml-auto text-light" type="submit" name="back" value="BACK" onClick="Javascript:window.location.href = 'user2_dashboard.php';" >
				<?php
				}
				?>
			   <?php
			...
			{
				$query = "select * from customer where REFERENCE_ID = '$_POST[rid]'";
				$query_run = mysqli_query($connection,$query);
				$c=1;
				while ($row = mysqli_fetch_assoc($query_run)) 
				{
			...
			if(isset($_POST['search_data']))
			{
				...
				?>
					...
					<form action="user2new_tran.php" method="post" autocomplete="off">	  	  
					<table>
					<tr>
			   ...
			   {
				$query = "insert into user2attend values('',curtime(),curdate(),'IN')";
				$query_run = mysqli_query($connection,$query);
			   ?>
			   <script type="text/javascript">
			   ...
			   {
				$query = "insert into user2attend values('',curtime(),curdate(),'OUT')";
				$query_run = mysqli_query($connection,$query);
			   ?>
			   <script type="text/javascript">
